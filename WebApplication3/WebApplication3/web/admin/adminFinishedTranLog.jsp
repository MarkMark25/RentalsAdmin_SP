...
try{
  connection = DriverManager.getConnection(connectionUrl+database, userid, password);
  statement=connection.createStatement();
  String sql ="Select c.reservation_number, concat(a.firstname,' ',a.lastname) as 'cName', b.name, b.rental_price, c.pickup_date, c.status from client a join transaction c using(client_id) join product b using(product_code) "
          + "where c.admin_id = '"+session.getAttribute("adminID")+"' and c.status='Success' or c.status='Terminated'";
