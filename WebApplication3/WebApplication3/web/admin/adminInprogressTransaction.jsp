<%-- 
    Document   : adminInprogressTransaction
    Created on : Nov 29, 2018, 12:48:27 PM
    Author     : mark_
--%>
...
try{
  ...
  String sql ="Select c.reservation_number, concat(a.firstname,' ',a.lastname) as 'cName', b.name, b.rental_price, c.pickup_date from client a join transaction c using(client_id) join product b using(product_code) "
          + "where c.admin_id = '"+session.getAttribute("adminID")+"' and c.status='InProgress'"; 
  int i=0;
  resultSet = statement.executeQuery(sql);
...
%>
                    ...
                    <tr>
                      ...
                      <td><a href='successTransac.jsp?d=<%=resultSet.getString("reservation_number")%>'> <button type="button" class="btn btn-default btn-xs btn-filter"><span style="color: green;" class="fas fa-user-check"></span></button></a>
                          <a href='terminateTransac.jsp?d=<%=resultSet.getString("reservation_number")%>'> <button type="button" class="btn btn-default btn-xs btn-filter"><span style="color: red;" class="fas fa-user-lock"></span></button></a></td>
                    </tr>
 <%
    i++;
}
